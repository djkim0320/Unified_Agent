...
function createHarness() {
  ...
  const browserRuntime = {
    ...
    async press() {
      return { url: "https://example.com", title: "Example", key: "Enter" };
    },
    async waitFor() {
      return { url: "https://example.com", title: "Example", matched: ["selector:main"] };
    },
    async screenshot() {
      return {
...
describe("runAgentTurn", () => {
  ...
  it("keeps strict JSON instructions when a tool registry is active", async () => {
    ...
    expect(instructionCalls[0]).toContain("browser_wait_for");
    expect(instructionCalls[0]).toContain("browser_screenshot");
  ...
  it("captures browser screenshots as workspace artifacts through the tool registry", async () => {
    const harness = createHarness();
    const adapter = createAdapter([
      {
        kind: "step",
        value: {
          ...
          tool: {
            ...
            arguments: {
              path: "artifacts/browser/check.png",
              fullPage: true,
            },
          },
        },
    ...
    expect(result.changedFiles).toContain("artifacts/browser/check.png");
    const saved = fs.readFileSync(
      path.join(harness.workspace.getSandboxDir(harness.conversationId), "artifacts", "browser", "check.png"),
    );
