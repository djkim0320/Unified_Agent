...
const ToolArgumentSchemas: Record<ToolName, z.ZodType<Record<string, unknown>>> = {
  ...
  browser_wait_for: BrowserWaitSchema,
...
function buildToolInstructions(extraGuide?: string) {
  const lines = [
    ...
    '- {"type":"tool_call","tool":{"name":"browser_press","arguments":{"selector":"textarea","key":"Control+Enter"}}}',
    '- {"type":"tool_call","tool":{"name":"browser_wait_for","arguments":{"selector":"main","timeoutMs":10000}}}',
    '- {"type":"tool_call","tool":{"name":"browser_screenshot","arguments":{"path":"artifacts/browser/home.png","fullPage":true}}}',
    ...
    '- {"type":"tool_call","tool":{"name":"create_task_flow","arguments":{"title":"Aircraft concept study","steps":[{"stepKey":"requirements","title":"Capture requirements","prompt":"Capture requirements, constraints, and assumptions. Expected output: requirements summary and unresolved questions."},{"stepKey":"research","title":"Research references","prompt":"Research relevant references. Expected output: cited findings and evidence gaps.","dependencyStepKey":"requirements"},{"stepKey":"decision-log","title":"Record decision log","prompt":"Compare options and record the decision. Expected output: decision log with rationale.","dependencyStepKey":"research"}]}}}',
    '- {"type":"final_answer"}',
    "Use only one tool per step.",
    "For long-running or multi-stage work, prefer create_task_flow over a single huge foreground answer.",
    "Each create_task_flow step prompt must include the expected output, artifact, decision, or blocker summary.",
...
async function executeTool<K extends ProviderKind>(params: {
  ...
  switch (params.toolCall.name) {
    ...
    case "browser_press": {
      return await params.browserRuntime.press({
        sessionId: params.runId,
        conversationId: params.conversationId,
        selector: typeof args.selector === "string" ? args.selector : undefined,
    ...
    case "browser_wait_for": {
      return await params.browserRuntime.waitFor({
        sessionId: params.runId,
        conversationId: params.conversationId,
        selector: typeof args.selector === "string" ? args.selector : undefined,
    ...
    case "browser_screenshot": {
      const capture = await params.browserRuntime.screenshot({
        sessionId: params.runId,
        conversationId: params.conversationId,
        fullPage: Boolean(args.fullPage),
        signal: params.signal,
      });
      const relativePath = params.workspace.writeBinaryFile({
        conversationId: params.conversationId,
        scope: "sandbox",
