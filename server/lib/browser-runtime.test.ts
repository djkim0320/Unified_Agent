...
function createFakePage(
  ...
  return {
    ...
    async waitForURL() {
      if (overrides?.waitForURL) {
        await overrides.waitForURL();
        return;
      }
      return undefined;
    },
...
describe("browser runtime", () => {
  ...
  it("supports wait, keyboard press, and screenshot computer-use actions", async () => {
    const stub = createLaunchBrowserStub();
    const runtime = createBrowserRuntime({
      launch: stub.launchBrowser as never,
      idleTimeoutMs: 60_000,
    ...
    await expect(
      ...
      matched: ["selector:main", "text:Example", "url:example.com"],
    });
    ...
    const capture = await runtime.screenshot({
      sessionId: "run-computer-use",
      conversationId: "conversation-a",
      fullPage: true,
    });
    expect(capture.mimeType).toBe("image/png");
    expect(capture.buffer.toString()).toBe("fake-png");
  });
});
