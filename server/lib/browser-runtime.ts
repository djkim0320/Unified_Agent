...
function boundedTimeout(timeoutMs: number | undefined, fallback = 10_000) {
  if (typeof timeoutMs !== "number" || !Number.isFinite(timeoutMs)) {
    return fallback;
  }
  return Math.min(Math.max(Math.trunc(timeoutMs), 250), 30_000);
}
...
export function createBrowserRuntime(options?: {
  ...
  async function press(params: SessionParams & { selector?: string; key: string; signal?: AbortSignal }) {
    const session = await ensureSession(params);
    return await runAbortableSessionOperation(params, async () => {
      if (params.selector) {
  ...
  async function waitFor(params: SessionParams & {
    selector?: string;
    text?: string;
    urlIncludes?: string;
    timeoutMs?: number;
    ...
    return await runAbortableSessionOperation(params, async () => {
      ...
      if (params.selector) {
        ...
        matched.push(`selector:${params.selector}`);
      }
      if (params.text) {
        await session.page.getByText(params.text).first().waitFor({ state: "visible", timeout });
        matched.push(`text:${params.text}`);
      }
      if (params.urlIncludes) {
        await session.page.waitForURL((url) => url.toString().includes(params.urlIncludes ?? ""), {
          timeout,
        });
        matched.push(`url:${params.urlIncludes}`);
      }
      await assertSafeNavigationUrl(session.page.url());
      session.lastUsedAt = Date.now();
      return {
  ...
  async function screenshot(params: SessionParams & { fullPage?: boolean; signal?: AbortSignal }) {
    const session = await ensureSession(params);
    return await runAbortableSessionOperation(params, async () => {
